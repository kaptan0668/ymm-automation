...
def parse_contract_text(text: str) -> dict:
    ...
    name = re.search(r"(?i)(musteri|unvan|mukellef)\s*[:\-]?\s*(.+)", text)
    if name:
    ...
    contract_no = re.search(r"(?i)(sozlesme\s*no)\s*[:\-]?\s*([\w/-]+)", text)
    if contract_no:
        data["contract_no"] = contract_no.group(2).strip()

    contract_type = re.search(r"(?i)(sozlesme\s*turu)\s*[:\-]?\s*(.+)", text)
    if contract_type:
